...
-(instancetype)init{
    ...
    if (self = [super init]) {
        //初始化写入文件对象（NSFileHandle 用于写入二进制）
        [self setFilHandle];
...
-(void)setFilHandle{
    ...
    //获取沙盒路径
    NSString * file = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES).lastObject stringByAppendingPathComponent:@"VideoToolbox.h264"];
    // 2 如果有原来的文件 删除
...
-(void)setVideoSession{
    ...
    // 6.设置码率(码率: 编码效率, 码率越高,则画面越清晰, 如果码率较低会引起马赛克 --> 码率高有利于还原原始画面,但是也不利于传输) 分辨率
...
-(void)enCoderSampleBuffer:(CMSampleBufferRef)sampleBuffer{
    ...
    //1 将sampleBuffer转为imageBuffer
    CVImageBufferRef imageBuffer = (CVImageBufferRef)CMSampleBufferGetImageBuffer(sampleBuffer);
    
    //更具当前的帧数 创建时间
    CMTime pressentationTimeStamp = CMTimeMake(self.frameId++, 1000);
    VTEncodeInfoFlags flags;
    
    //开始编码
    OSStatus statusCode = VTCompressionSessionEncodeFrame(self.compressionSession, imageBuffer, pressentationTimeStamp, kCMTimeInvalid, NULL, (__bridge void * _Nullable)(self), &flags);
